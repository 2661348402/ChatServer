...
void sha256_transform(const unsigned char* data, uint32_t* state) {
...
std::string SHA256::hash(const std::string& input) {
    ...
    size_t totalBytes = input.size();
    size_t processed = 0;

    while (processed + 64 <= totalBytes) {
        std::memcpy(block, input.data() + processed, 64);
    ...
    std::memcpy(block, input.data() + processed, remaining);
