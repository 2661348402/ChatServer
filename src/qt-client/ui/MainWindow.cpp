    ...
    , m_client(client)
{
    ...
    // ================================================================
    //  Left panel — ContactPanel (search, friends, groups, + menu)
    // ================================================================
    m_contactPanel = new ContactPanel(this);
    central->addWidget(m_contactPanel);
    ...
    central->addWidget(m_tabWidget);

    // Proportion: ~28% left, 72% right
...
void MainWindow::onReconnectTimer()
{
    ...
    m_statusLabel->setText(
        QString("Reconnecting (%1/%2)...")
            .arg(m_reconnectRetries).arg(MAX_RECONNECT_RETRIES));
...
void MainWindow::restyle()
{
    ...
    // Status label
    m_statusLabel->setStyleSheet(
        QString("color: %1; font-size: 12px;").arg(
            m_reconnectTimer->isActive() ? Theme::textError() : Theme::textSecondary()));
