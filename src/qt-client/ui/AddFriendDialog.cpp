...
AddFriendDialog::AddFriendDialog(ProtocolClient* client, QWidget* parent)
    : QDialog(parent)
    , m_client(client)
{
    setWindowTitle("ChatServer — Add Friend");
    ...
    // Title
    auto* titleLabel = new QLabel("Add Friend", card);
    ...
    m_addBtn = new QPushButton("Add", card);
...
{
    m_client->sendAddFriend(m_idSpin->value());
    accept();
}
